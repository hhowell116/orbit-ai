...
### Google Drive Integration (Shared Drive → local projects)

Orbit AI projects can be backed by folders in the IT Applications → **Dev
Projects** shared drive. The goal: edits made in Orbit AI flow up to Drive, and
edits made by teammates in Drive flow down to Orbit AI — with local-filesystem
speed inside the broker (no FUSE hangs).

**Architecture:**

```
  ~/gdrive/                             ← rclone FUSE mount (browse only)
    └── RCO Rebrand Hub/                 — direct live view of Drive
        ├── ...

  ~/orbit-ai/projects/rco-rebrand-hub/  ← REAL local directory (what Orbit AI uses)
    ├── 0-design-principles.md           — synced via rclone bisync every 5 min
    ├── CLAUDE.md                        — local-only (auto-generated by broker)
    └── ...
```

The rclone mount exists for ad-hoc browsing from a terminal. **Orbit AI projects
never point at the mount** — they point at a real local directory that is kept
in sync with the Drive folder by a systemd timer.

**Why not symlink the project path into the mount directly?**
That was tried once. The broker's filewatcher recursed into the FUSE mount,
opened 468+ file descriptors against Google Drive, and every inotify event
required a round-trip through the Drive API. The event loop drowned, HTTP
requests hung while TCP was still accepting, and the site returned 530. The
`sync-drive-project.sh` script refuses to run against a symlink for this reason.

**Components:**

- `~/.local/bin/rclone` — v1.73+ binary, user-local install
- `~/.config/rclone/rclone.conf` — remote `gdrive:` rooted at the Dev Projects
  folder (`team_drive=0AA_2e2_CkzvqUk9PVA`, `root_folder_id=1A8w4-...`)
- `~/.config/rclone/bisync-filters.txt` — excludes `node_modules`, `.git`,
  `dist`, `build`, `.cache`, `__pycache__`, `CLAUDE.md`, etc. so Drive stays
  clean and only source files sync
- `rclone-gdrive.service` — keeps `~/gdrive/` FUSE-mounted (browse only)
- `orbit-bisync-<slug>.service` + `.timer` — per-project bisync, one pair
  per synced project, timer fires every 5 min
- `sync-drive-project.sh` — **adds a new Drive-synced project** (see below)

**Adding a new Drive-synced project:**
...
That single command:
1. Creates `/home/rowecasa/orbit-ai/projects/rco-launch-hub/`
2. Does an initial `rclone bisync --resync` (Drive → local, first-run baseline)
3. Writes `orbit-bisync-rco-launch-hub.service` + `.timer`
4. Enables the timer so it fires every 5 min
5. Prints the local path to paste into the Orbit AI dashboard as the project's path

Pass a second arg to override the auto-generated slug:

```bash
./sync-drive-project.sh "RCO Launch Hub" launch-hub
```
...
| Task | Command |
|------|---------|
| Force immediate sync | `systemctl --user start orbit-bisync-<slug>` |
| View sync logs | `journalctl --user -u orbit-bisync-<slug> -f` |
| Disable sync for a project | `systemctl --user disable --now orbit-bisync-<slug>.timer` |
| List all Drive-synced projects | `systemctl --user list-timers 'orbit-bisync-*'` |
| List available Drive folders | `./sync-drive-project.sh` (no args) |

**Conflict handling:** bisync uses `--conflict-resolve newer` so the file with
the latest modification time wins. The losing version is deleted (not renamed
to `.conflict1` — keeps the local tree clean). If two people edit the same
file within the same 5-min window, only the later save survives.

**OAuth token refresh:** rclone refreshes the Drive access token automatically
using the refresh token stored in `rclone.conf`. No manual intervention ever
needed unless Google revokes the token (rare; would require re-running the
`rclone authorize "drive"` flow from a laptop).
...
├── sync-drive-project.sh       ← Add a Drive folder as a bi-directionally synced Orbit AI project
│
...
│  ~/.local/bin/rclone                                    ← rclone binary (Google Drive client)
│  ~/.config/rclone/rclone.conf                           ← Drive OAuth + remote config
│  ~/.config/rclone/bisync-filters.txt                    ← Exclude list for Drive syncs
...
│  ~/.config/systemd/user/rclone-gdrive.service           ← FUSE mount at ~/gdrive (browse only)
│  ~/.config/systemd/user/orbit-bisync-<slug>.service     ← Per-project Drive bisync job
│  ~/.config/systemd/user/orbit-bisync-<slug>.timer       ← Fires bisync every 5 min
