...
interface FileNode {
  ...
  type: "file" | "dir";
  path: string;
  mtime?: number; // Unix timestamp ms — for detecting recently edited files
  children?: FileNode[];
}
...
function scanDir(absPath: string, rootPath: string, depth: number): FileNode[] {
  if (depth > 8) return [];
  let entries: { name: string; isDir: boolean }[];
  try {
    entries = readdirSync(absPath, { withFileTypes: true }).map((d) => ({
      name: d.name,
      isDir: d.isDirectory(),
    }));
  } catch {
  ...
  for (const entry of entries) {
    ...
    if (!entry.isDir && IGNORED_FILES.has(entry.name)) continue;
    ...
    if (entry.isDir) {
      nodes.push({
        ...
        type: "dir",
        path: relPath,
        children: scanDir(fullPath, rootPath, depth + 1),
      });
    } else {
      let mtime: number | undefined;
      try { mtime = statSync(fullPath).mtimeMs; } catch {}
      nodes.push({ name: entry.name, type: "file", path: relPath, mtime });
    }
  }
...
teamApi.get("/projects/:id/files", (c) => {
  const user = c.get("user") as JWTPayload;
  const { id } = c.req.param();
  const project = db
    .query("SELECT path FROM projects WHERE id = ? AND team_id = ?")
    .get(id, user.team_id) as { path: string } | null;
  if (!project) return c.json({ error: "Project not found" }, 404);

  const tree = scanDir(project.path, project.path, 1);
  return c.json(tree);
});

teamApi.get("/projects/:id/claude-activity", (c) => {
  const { id } = c.req.param();
  const result: { file: string; action: string }[] = [];
  const cutoff = Date.now() - 15000;
  for (const [key, map] of sessionFileActivity) {
    if (key.includes(id)) {
      for (const [filePath, info] of map) {
        if (info.ts > cutoff) {
          result.push({ file: filePath, action: info.action });
        }
      }
    }
  }
...
import { startWatching, stopWatching, setFilewatcherBroadcast, setTerminalWarningFn } from "./filewatcher";
...
function scanClaudeActivity(sessionKey: string, raw: string) {
  // Buffer output so we catch tool patterns split across chunks
  let buffer = (sessionOutputBuffers.get(sessionKey) || "") + raw;
  if (buffer.length > 4096) buffer = buffer.slice(-4096);
  sessionOutputBuffers.set(sessionKey, buffer);

  const text = stripAnsi(buffer);
  let map = sessionFileActivity.get(sessionKey);
  if (!map) { map = new Map(); sessionFileActivity.set(sessionKey, map); }

  // Match Claude Code tool patterns — multiple formats across versions:
  // "Read(src/file.tsx)" "Edit(src/file.tsx)" "Write(src/file.tsx)"
  // "⎿  Read src/file.tsx" "Read file_path" etc.
  // Also match file paths with common extensions anywhere in output
  const toolRegex = /\b(Read|Edit|Write|Grep|Glob)\s*[\(]?\s*([^\s\)\,\]]+\.[a-zA-Z]{1,10})/gi;
  let m;
  while ((m = toolRegex.exec(text)) !== null) {
    const tool = m[1].toLowerCase();
    const filePath = m[2].replace(/['"`;:]+$/, "").replace(/\\/g, "/");
    const action = (tool === "read" || tool === "grep" || tool === "glob") ? "reading" : "editing";
    map.set(filePath, { action, ts: Date.now() });
  }

  // Also match standalone file paths being referenced (e.g. "src/components/App.tsx")
  // Look for paths with at least one / and a file extension
  const pathRegex = /(?:^|[\s\(])((?:[\w.-]+\/)+[\w.-]+\.[a-zA-Z]{1,10})\b/gm;
  while ((m = pathRegex.exec(text)) !== null) {
    const filePath = m[1].replace(/\\/g, "/");
    // Only track if not already tracked with a specific action
    if (!map.has(filePath)) {
      map.set(filePath, { action: "reading", ts: Date.now() });
    }
  }

  // Clean up entries older than 15 seconds
  const cutoff = Date.now() - 15000;
  for (const [path, info] of map) {
    if (info.ts < cutoff) map.delete(path);
  }
}
...
function sendTerminalWarning(userId: string, projectId: string, message: string) {
  ...
  if (!clients) return;
  const warning = `\r\n\x1b[41;37;1m  LOCKED  \x1b[0m \x1b[31m${message}\x1b[0m\r\n`;
  for (const client of clients) {
    try { client.send(JSON.stringify({ type: "output", data: warning })); } catch {}
  }
}

// Give the filewatcher access to send terminal warnings on lock conflicts
setTerminalWarningFn(sendTerminalWarning);
