...
export function ProjectPage() {
  ...
  // Poll sidebar data (5s for general data, 2s for live Claude activity + file tree)
  useEffect(() => {
    if (!projectId) return;
    ...
    function refreshFast() {
      broker.getProjectFiles(projectId!).then(setFileTree).catch(() => {});
      broker.getClaudeActivity(projectId!).then(setClaudeActivity).catch(() => {});
    }
    refreshSlow();
    refreshFast();
    const slowInterval = setInterval(refreshSlow, 5000);
    const fastInterval = setInterval(refreshFast, 2000);
    ...
    <div className="h-screen flex flex-col" style={{ background: "var(--color-bg-base)", color: "var(--color-text-primary)" }}>
      ...
      <div className="flex-1 flex min-h-0">
        ...
        <div className="flex-[3] overflow-y-auto">
          ...
          {/* Project Files */}
          <div className="p-4" style={{ borderBottom: "1px solid var(--color-border)" }}>
            <h3 className="text-xs font-medium uppercase tracking-wider mb-3" style={{ color: "var(--color-text-muted)" }}>Project Files</h3>
            <div className="max-h-64 overflow-y-auto" style={{ marginRight: -4, paddingRight: 4 }}>
              <FileTree
                files={fileTree}
                locks={locks}
                activity={(() => {
                  // Merge PTY-scanned activity with mtime-based edit detection
                  const merged = [...claudeActivity];
                  const seen = new Set(merged.map((a) => a.file));
                  const cutoff = Date.now() - 15000; // 15 seconds
                  function walk(nodes: any[]) {
                    for (const n of nodes) {
                      if (n.mtime && n.mtime > cutoff && !seen.has(n.path)) {
                        merged.push({ file: n.path, action: "editing" });
                        seen.add(n.path);
                      }
